...
public static class LogicMapper
{
    public static AssetLogicData MapLogicXml(XElement logicElement)
    {
        ...
        // Map planet systems
        var planetSystemElement = logicElement.Element("planetSystem")?.Element("objects");
        if (planetSystemElement != null)
        {
            output.PlanetSystems = MapPlanetSystems(planetSystemElement.Elements("object"));
        }
    ...
    private static List<AssetLogicPlanetSystem> MapPlanetSystems(IEnumerable<XElement> planetElements)
    {
        return planetElements.Select(p => new AssetLogicPlanetSystem
        {
            ...
            Radius = float.TryParse(p.Attribute("radius")?.Value, out var radius) ? radius : (float?)null,
            ArcSpeed = float.TryParse(p.Attribute("arcSpeed")?.Value, out var arcSpeed) ? arcSpeed : (float?)null,
            ArcOffset = float.TryParse(p.Attribute("arcOffset")?.Value, out var arcOffset) ? arcOffset : (float?)null,
            Blend = int.TryParse(p.Attribute("blend")?.Value, out var blend) ? blend : (int?)null,
            Height = float.TryParse(p.Attribute("height")?.Value, out var height) ? height : (float?)null
        }).ToList();
    }
}
